...
// 连接成功建立的回调方法
nzqGameWebSocket.onopen = function() {
	myxf = $("#id_container").html();
	nzqGameWebSocket.send("01" + myxf);
};

// 接收到消息的回调方法
nzqGameWebSocket.onmessage = function(event) {
	var msg = event.data;
//	alert("nzq收到消息"+msg);
...
//		alert(msg);
	...
	case "08":
		...
		break;
	case "09":
		ReceiveRequestChangeLocation();
		break;
	case "10":
		changeLocationBetweenTwoPlayerDealer(msg);
		break;
	case "11":
		beginGy5();
		break;
	...
	case "23":
		...
		break;
	case "24":
		ReceiveRequestChangeLocation3(msg);
		break;
	case "25":
		changeLocationBetweenTwoPlayer3(msg);
		break;
	case "26":
		beginGy4();
		break;
	case "27":
		...
		break;
	case "28":
		receiveInnerMsg3(msg);
		break;
	case "29":
		receiveEmoticon3(msg);
		break;
	case "30":
		receiveVoice3(msg);
		break;
	case "32":
		enterMatchPreparement(msg);
		break;
	case "33":
		match5Confirm();
		break;
